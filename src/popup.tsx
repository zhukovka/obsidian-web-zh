  ...
  useEffect(() => {
    ...
    useEffect(() => {
        ...
        previewContext.page.content =
            ...
            try {
                const _metaScript = await chrome.scripting.executeScript({
                    target: {tabId: tab.id},
                    func: () => {
                        function extractTags(keywords: string) {
                        ...
                        const supportedTags = ["description", "keywords"]
                        ...
                        if (image) {
                            ...
                            if (content) {
                                cover = `[![thumbnail](${content})](${document.documentURI})\n`
                            }
                        }

                        const tags = Array.from(nodes).filter(n => {
                            ...
                            let name = n.getAttribute("name");
                            let content = n.getAttribute("content");
                        ...
                        return `${cover}${tags.join("\n")}`;
                    },
                });
                _metaTags = _metaScript[0].result;
                console.log("_metaTags", _metaTags);
            } catch (e) {
            }
